  ...
  {#each rowData as row (row.idx)}
    <div class="row" class:sys={row.sys} class:route={!row.sys && row.outboundKind === 'route'}>
      ...
      <div class="actions-col actions">
        {#if !row.sys}
          <button type="button" class="icon-btn" title="Редактировать" onclick={() => onEdit(row.idx)}>
            <Edit3 size={12} />
          </button>
          <button type="button" class="icon-btn danger" title="Удалить" onclick={() => onDelete(row.idx)}>
            <Trash2 size={12} />
          </button>
        {/if}
      </div>
...
<style>
  ...
  .header, .row {
    display: grid;
    grid-template-columns: 40px 60px 90px 1fr 180px 80px;
    align-items: center;
    gap: 8px;
    padding: 8px 14px;
  }
